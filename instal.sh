#!/bin/bash
# Safe tput wrapper (fallback ke nilai default)
safe_tput() {
    local type="$1"
    local def="$2"
...
pulse() {
    for i in {1..20}; do
        printf "\033[48;2;255;0;0m%*s\033[0m\r" "$(safe_tput cols 80)" ""
        sleep 0.025
        printf "\033[48;2;30;0;0m%*s\033[0m\r" "$(safe_tput cols 80)" ""
        sleep 0.025
    done
}
...
chroma_swirl() {
    for i in {1..60}; do
        printf "\033[48;2;$((i*3%255));$((i*7%255));$((i*11%255))m%$(safe_tput cols 80)s\033[0m\r" ""
        sleep 0.02
    done
    echo
}
...
flicker_grid() {
    for i in {1..40}; do
        for c in $(seq 1 $(safe_tput cols 80)); do
            printf "\033[38;5;$((RANDOM%200))m#\033[0m"
        done
        echo
        sleep 0.02
...
flicker_curtain() {
    for i in {1..40}; do
        printf "\033[48;2;$((RANDOM%255));$((RANDOM%255));$((RANDOM%255))m%*s\033[0m\r" "$(safe_tput cols 80)" ""
        sleep 0.008
    done
    echo
}
...
energy_bars() {
    for i in {1..30}; do
        level=$((RANDOM % $(safe_tput cols 80)))
...
chaotic_lines() {
    for i in {1..80}; do
        echo -e "\033[38;5;$((RANDOM%255))m"$(printf '%*s' $((RANDOM%$(safe_tput cols 80))) | tr ' ' '-')"\033[0m"
        sleep 0.01
    done
}
...
scanline() {
    for i in {1..12}; do
        printf "\033[48;2;10;10;10m%$(safe_tput cols 80)s\033[0m\r"
        sleep 0.02
        printf "\033[48;2;0;0;0m%$(safe_tput cols 80)s\033[0m\r"
        sleep 0.02
    done
}
