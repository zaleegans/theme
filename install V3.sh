...
glitch() {
    txt="$1"
    for i in {1..6}; do
        printf "\033[35m$txt\033[0m\r"
        sleep 0.05
        printf "\033[31m$txt\033[0m\r"
        sleep 0.05
    done
    printf "$txt\n"
}
...
shake() {
    for i in {1..20}; do
        printf "\033[2J\033[$((RANDOM%10));$((RANDOM%10))H⚠️"
        sleep 0.03
    done
    clear
}
...
rgb_wall() {
    for i in {1..20}; do
        printf "\033[48;2;$((RANDOM%255));$((RANDOM%255));$((RANDOM%255))m%$(tput cols)s\033[0m\n"
        sleep 0.01
    done
}
